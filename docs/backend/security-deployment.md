...
- Preserve the ADR 0014 subject order selected from the first trade and institution profile:
  `<FCN|DAC|DRA>(T+7) <branch?> [RFQ:...][BATCH:...]`; never append the legacy ` DAC/DRA`
  marker.
- Never use `##`.
