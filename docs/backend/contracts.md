...
Generated quote images are based only on a finalized ranking snapshot. Rank-one trades are grouped by issuer. An artifact response exposes metadata and an authenticated download endpoint, never the private R2 object key or a permanent public URL.
