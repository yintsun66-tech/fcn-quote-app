...
提供，並透過 `backend-client.js` 核心加入登入、可勾選發行機構的後端自動寄信、回覆解析、
15 分鐘報價期加 60 秒郵件轉送緩衝、經濟前四名加自選第五家比價、晚到報價版本化
重新排名、隨選報價圖（在使用者瀏覽器產生，DAC 產品加註浮動收益）、可恢復的
「我的詢價」工作區，以及 ADMIN／PS 管理功能（所有帳號列表與上次上線時間、
升級／降級 PS、剔除一般帳號、刪除已剔除帳號的個資並匿名保留歷史詢價、
使用者申請審核、重複申請提示、以行編查詢帳號）。一般使用者忘記密碼時可重置為
30 分鐘有效的臨時密碼，登入後必須立即修改；ADMIN／PS 不適用自助重置。
建立、驗證與排入寄信已合併為一次前端請求；選擇三家以內時會顯示實際寄信封數，
全部回覆仍立即完成，若七分鐘後每筆至少已有 `min(2, 已選家數)` 家有效報價，
使用者可選擇提早完成。
未勾選機構即使因共用 BMJB 郵件回覆，也只保留稽核資料，不會進入排名或產圖。
新帳號申請只需分行名稱、五碼行編與密碼；核准後以五碼行編登入。既有帳號仍沿用
原本的登入名稱，不會被自動改名。

為縮短手機啟動時間，ADMIN、風險分析及報價圖程式已拆成第一次使用時才下載的
`backend-admin.mjs`、`backend-analysis.mjs` 與 `backend-image.mjs`；跟單專區維持獨立
`follow-board.mjs`。GitHub Pages 靜態版不會下載上述後端模組。自架的 html2canvas 也
只在使用者按下產圖時載入。一般 API 有明確逾時，財報標的查詢會合併相同請求並取消
已被新輸入取代的舊請求。
...
- 選擇三家以內不會自行縮短期限；每筆在七分鐘後至少有 `min(2, 已選家數)` 家有效報價時，
  介面才會醒目提示既有的提早完成操作。等待中的小型詢價最慢每 8 秒更新一次。
- 外寄 Queue concurrency 為 8；ADMIN 的 RFQ 時間軸會顯示建立→排隊、排隊→首封、
  首封→末封及排隊→全部寄完時間，並彙總三家以下是否在 20 秒內寄完。10～20 秒是
  部署後以正式樣本驗證的操作目標，不是發行機構回覆保證。
...
- 目前驗證基線為 27 個測試檔、198 項測試；JavaScript 語法、TypeScript typecheck、
